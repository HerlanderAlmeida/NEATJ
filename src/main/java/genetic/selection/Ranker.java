...
public final class Ranker<T extends Individual, R>
{
	...
	{
		return toRank.sorted(this.order);
	}
	...
	{
		return StreamSupport.stream(toRank.spliterator(), false).sorted(this.order);
	}
	 ...
	 * @return A ranked {@link Stream} of evaluations
	 */
	public Stream<Evaluation<T, R>> rank(Evaluator<T, R> evaluator, Stream<T> ts)
	{
		return rank(evaluator.evaluate(ts));
	}
}
